...
void sub_Topics(){
	 ...
	 subscribe_topics[3] = String(config.deviceID) + "/State";
}
...
String relayState;
    ...
    ChangeRelayState(server.arg("state"));
    server.send(200, "<! DOCTYPE json>", "<html><head><meta http-equiv=\"Content-Language\" content=\"en-ca\"><meta name=\"viewport\" content=\"width=device-width, user-scalable=no\"head><body>State has been changed</body></html>" );
}
...
void TraiterMessageRecu(String topic, String payload) {
 ...
 if (topic == subscribe_topics[3]) { ChangeRelayState(payload); }
}
...
// Change Relay State
//********************************************************************************
void ChangeRelayState(String payload) {
	if (payload == "On"  or payload == "1") { digitalWrite(relay,HIGH); }
	if (payload == "Off" or payload == "0") { digitalWrite(relay,LOW);  }
	PublishRelayStatus();
}
...
String strState = String(relayState);

	Serial.print("Publishing relay status ");
	Serial.println(relayState);
	client.publish(config.deviceID, strState.c_str());
}
